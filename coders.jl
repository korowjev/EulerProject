...
function getproblem(problem::Int, pair::Int, shifts::Int)
    infile = open("./coded_problems/coded_problem_$problem.txt", "r")
    codedproblem = readall(infile)
    close(infile)
    uncodedproblem = decode(pair, shifts, codedproblem)
    outfile = open("./uncoded_problems/uncoded_problem_$problem.jl", "w")
    write(outfile, uncodedproblem)
    close(outfile)
end

function codeproblem(problem::Int, pair::Int, shifts::Int)
    infile = open("./uncoded_problems/uncoded_problem_$problem.jl", "r")
    uncodedproblem = readall(infile)
    close(infile)
    outfile = open("./coded_problems/coded_problem_$problem.txt", "w")
